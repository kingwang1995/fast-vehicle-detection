...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_weights", '-pre', type=str,
                        default="", help="if specified starts from checkpoint model")
    ...
    parser.add_argument('--learning_rate', '-lr', dest='lr', type=float, default=1e-4, help='initial learning rate')
    ...
    for epoch in range(opt.epochs):
        ...
        if epoch % opt.checkpoint_interval == 0 or epoch == opt.epochs - 1:
            torch.save(model.state_dict(), f"checkpoints/fvd_ckpt_{epoch}_{timestamp}.pth")
